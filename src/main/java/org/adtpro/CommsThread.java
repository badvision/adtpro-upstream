...
public class CommsThread extends Thread {
    ...
    public void dispatchCommand(byte[] envelope) {
        ...
        switch (envelope[2]) {
            ...
            case (byte) 205: // "M": Multiple Nibble send
                ...
                receiveNibbleDiskWide(true, envelope);
                /* Assume 35 tracks */
                _busy = false;
                break;
            case (byte) 206: // "N": Put Nibble Disk
    ...
    public void receiveDisk(boolean generateName) /* Main receive routine - Host <- Apple (Apple sends) */ {
    ...
    public void receiveDiskWide(boolean generateName, byte[] envelope) /* Main receive routine - Host <- Apple (Apple sends) */ {
    ...
    public void sendDisk() /* Main send routine - Host -> Apple (Host sends) */ {
    ...
    public void sendDiskWide(byte[] envelope) /* Main send routine - Host -> Apple (Host sends) */ {
    ...
    public boolean sendPacket(byte[] buffer, int block, int offset, int preambleStyle) // Send a packet with RLE compression
    // preambleStyle:
    // 0 = No preamble
    // 1 = ProDOS packets
    // 2 = Nibble packets
    ...
    public boolean sendPacketWide(byte[] buffer, int block, int blocksAtOnce) // Send a packet with RLE compression
    {
        return sendPacketWide(buffer, block, blocksAtOnce, 15);
    }

    public boolean sendPacketWide(byte[] buffer, int block, int blocksAtOnce, int timeoutSuggestion) // Send a packet with RLE compression
    {
        boolean rc = false;
    ...
    public void sendNibbleDisk(boolean generateName) /* Nibble send routine - Apple <- Host (Host sends) */ {
    ...
    public void receiveNibbleDisk(boolean generateName, int requestedTracks) /* Nibble receive routine - Host <- Apple (Apple sends) */ {
        boolean shouldContinue = true;
 ...
 /*
    ...
    public void receiveNibbleDiskWide(boolean generateName, byte[] envelope) /* Nibble receive routine - Host <- Apple (Apple sends) */ {
        boolean shouldContinue = true;
    ...
    public int receivePacket(byte[] buffer, int offset, int buffNum, int preambleStyle) // Receive a packet with RLE compression
    // preambleStyle:
    // 0 = No preamble
    // 1 = ProDOS packets
    // 2 = Nibble packets
    ...
    public int receivePacketWide(byte[] buffer, int offset, int expectedStartBlock) throws GoHomeException // Receive an enveloped wide packet with RLE compression
    // Returns:
    ...
    void makeCrcTable() /* Fill the crctable[] array needed by doCrc */ {
        int oneByte, bit;
        int crc;
