...
public class CaptureThread extends Thread {
    ...
    void interpretStream(byte buffer[]) {
        for (int i = 0; i < buffer.length; i++) {
            ...
            if ((transitionValue >= 0) && (buffer[i] >= transitionValue)) {
                ...
                isTransition = true;
            } else if ((transitionValue < 0) && (buffer[i] <= transitionValue)) {
                transitionValue = transitionHysteresis;
                isTransition = true;
            } else {
                isTransition = false;
    ...
    public void flushReceiveBuffer() {
        if (receiveBufferSize() > 0) //if (receiveBufferSize() > 128)  I'm confused... why would we not just flush if > 0?
        {
