...
public class AudioTransport extends ATransport {
    ...
    public byte readByte(int timeout) throws TransportTimeoutException {
        ...
        if (_receiveBuffer != null) {
            /*
			 * if ((_inPacketPtr <= _receiveBuffer.length) && (_receiveBuffer.length > 0)) { int myByte = _receiveBuffer[_inPacketPtr]; if (myByte < 0) myByte += 256; Log.println(false, "AudioTransport.readByte() exit with " + UnsignedByte.toString(UnsignedByte.loByte(myByte))); }
             */
    ...
    public void pullBuffer(int seconds) throws TransportTimeoutException, TransportClosedException {
        ...
        while ((_captureThread != null) && ((_captureThread.receiveBufferSize()) == 0) && (collectedTimeouts / 4 < seconds)) {
            // Log.println(false, "AudioTransport.pullBuffer() sleeping... collectedTimeouts: " + collectedTimeouts / 4 + " requested: " + seconds);
            try {
                Thread.sleep(250);
            } catch (InterruptedException e) {
    ...
    public String getInstructions(String guiString, int fileSize, int serialSpeed) {
        ...
        } else if ((guiString.equals(Messages.getString("Gui.BS.ADTPro"))) || (guiString.equals(Messages.getString("Gui.BS.ADTProAudio"))) || (guiString.equals(Messages.getString("Gui.BS.ADTProEthernet")))) {
    ...
    public String getInstructionsDone(String guiString) {
        ...
        } else if ((guiString.equals(Messages.getString("Gui.BS.ADT"))) || (guiString.equals(Messages.getString("Gui.BS.ADTPro"))) || (guiString.equals(Messages.getString("Gui.BS.ADTProAudio"))) || (guiString.equals(Messages.getString("Gui.BS.ADTProEthernet")))) {
